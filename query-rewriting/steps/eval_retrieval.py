...
def query_similar_docs(
    ...
    if vector_store_name == "elasticsearch":
...
def query_similar_docs_with_expansion(
    question: str,
    url_ending: str,
    model: str = OPENAI_MODEL,
    n_items_per_query: int = 5,
    returned_sample_size: int = 5,
) -> Tuple[str, str, List[str]]:
    """Query similar documents using query expansion for a given question and URL ending.
    
    This function uses query rewriting to generate multiple variations of the original query,
    retrieves documents for each variation, and combines the results.

    Args:
        question: Question to query similar documents for.
        url_ending: URL ending to compare the retrieved documents against.
        model: Model to use for query rewriting. Defaults to OPENAI_MODEL.
        n_items_per_query: Number of documents to retrieve per query. Defaults to 5.
        returned_sample_size: Number of documents to return after reranking. Defaults to 5.
    ...
    # Add the original query to ensure it's included
    if question not in rewritten_queries:
        rewritten_queries = [question] + rewritten_queries[:-1]
    ...
    for query in rewritten_queries:
        ...
        similar_docs = get_topn_similar_docs(
            ...
            conn=client if vector_store_name != "elasticsearch" else None,
            es_client=client if vector_store_name == "elasticsearch" else None,
            n=n_items_per_query,
            include_metadata=True,
        )
    ...
    results = ranker.rank(query=question, docs=docs_texts)  # Using original query for final ranking
    
    # Extract URLs from reranked documents
    reranked_docs = []
    for result in results.results:
        index_val = result.doc_id
        doc_url = unique_docs[index_val][1]
        reranked_docs.append(doc_url)
    
    return (question, url_ending, reranked_docs[:returned_sample_size])
...
def process_single_pair_with_expansion(
    pair: Dict, model: str = OPENAI_MODEL
) -> Tuple[bool, str, str, List[str]]:
    """Process a single question-document pair using query expansion.
    ...
    Returns:
        Tuple containing (is_failure, question, url_ending, retrieved_urls)
    """
    question, url_ending, urls = query_similar_docs_with_expansion(
        pair["question"], pair["url_ending"], model=model
    )
    is_failure = all(url_ending not in url for url in urls)
    return is_failure, question, url_ending, urls
...
def process_single_pair_with_expansion_retry(
    pair: Dict, model: str = OPENAI_MODEL
) -> Tuple[bool, str, str, List[str]]:
    """Process a single question-document pair with query expansion and retry logic.
    ...
    """
    try:
        return process_single_pair_with_expansion(pair, model)
    except Exception as e:
        logging.warning(
            f"Error processing pair with expansion {pair['question']}: {str(e)}. Retrying..."
        )
        raise


def perform_small_retrieval_evaluation_with_expansion(model: str = OPENAI_MODEL) -> float:
    """Helper function to perform the small retrieval evaluation with query expansion.
    ...
    """
    total_tests = len(question_doc_pairs)
    logger.info(f"Starting retrieval test with query expansion on {total_tests} questions...")

    n_processes = max(1, cpu_count() // 2)
    worker = partial(
        process_single_pair_with_expansion_retry, model=model
    )
    ...
    try:
        results = process_with_progress(
            ...
            worker,
            n_processes,
            "Testing document retrieval with query expansion",
        )
...
@step
def retrieval_evaluation_small_with_expansion(
    model: str = OPENAI_MODEL,
) -> Annotated[float, "small_failure_rate_retrieval_expansion"]:
    """Executes the retrieval evaluation step with query expansion.

    Args:
        after: Filter for model versions after this date. Defaults to None.
        model: Model to use for query rewriting. Defaults to OPENAI_MODEL.

    Returns:
        The failure rate of the retrieval test.
    """
    failure_rate = perform_small_retrieval_evaluation_with_expansion(model=model)
    logging.info(f"Retrieval failure rate with query expansion: {failure_rate}%")
    return failure_rate


def process_single_dataset_item_with_expansion(
    item: Dict, model: str = OPENAI_MODEL
) -> Tuple[bool, str, str, List[str]]:
    """Process a single dataset item with query expansion.
    ...
    generated_questions = item["generated_questions"]
    question = generated_questions[0]  # Assuming only one question per item
    url_ending = item["filename"].split("/")[-1]  # Extract the URL ending from the filename

    _, _, urls = query_similar_docs_with_expansion(question, url_ending, model=model)
    is_failure = all(url_ending not in url for url in urls)
    return is_failure, question, url_ending, urls
...
def process_single_dataset_item_with_expansion_retry(
    item: Dict, model: str = OPENAI_MODEL
) -> Tuple[bool, str, str, List[str]]:
    """Process a single dataset item with query expansion and retry logic.
    ...
    """
    try:
        return process_single_dataset_item_with_expansion(item, model)
    except Exception as e:
        logging.warning(
            f"Error processing dataset item with expansion: {str(e)}. Retrying..."
        )
        raise


def perform_retrieval_evaluation_with_expansion(
    sample_size: int, model: str = OPENAI_MODEL
) -> float:
    """Helper function to perform the retrieval evaluation with query expansion.
    ...
    n_processes = max(1, cpu_count() // 2)
    worker = partial(
        process_single_dataset_item_with_expansion_retry, model=model
    )

    try:
        results = process_with_progress(
            sampled_dataset, worker, n_processes, "Evaluating retrieval with query expansion"
        )
...
@step
def retrieval_evaluation_full_with_expansion(
    sample_size: int = 100,
    model: str = OPENAI_MODEL,
) -> Annotated[float, "full_failure_rate_retrieval_expansion"]:
    """Executes the retrieval evaluation step with query expansion.
    ...
    Returns:
        The failure rate of the retrieval test.
    """
    failure_rate = perform_retrieval_evaluation_with_expansion(
        sample_size, model=model
    )
    logging.info(f"Retrieval failure rate with query expansion: {failure_rate}%")
    return failure_rate
